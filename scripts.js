...
const storageKey = "Mars-Code-Editor";
...
// Run native light linter when JS changes
ed_js.session.on("change", () => {
  lintJavaScript(ed_js);
});

attachCursorTracking(ed_html, "HTML", "index.html");
attachCursorTracking(ed_css, "CSS", "style.css");
attachCursorTracking(ed_js, "JavaScript", "script.js");

function attachCursorTracking(editor, lang, file) {
  const cursorPos = document.getElementById("cursorPos");
  const langMode = document.getElementById("langMode");
  const fileName = document.getElementById("fileName");

  langMode.textContent = lang;
...
function showPane(name) {
    TAB_ORDER.forEach(k => {
        if (wraps[k]) wraps[k].style.display = (k !== name) ? "none" : "block";
    });
    ...
    requestAnimationFrame(() => {
        ...
        if (ed && ed.resize) {
            ...
            attachCursorTracking(ed, langMap[name], fileMap[name]);
        }
    });
}
...
function lintJavaScript(editor) {
    const text = editor.getValue();
    const annotations = [];

    // Basic native syntax validator to prevent external CDN errors
    try {
        new Function(text);
    } catch (e) {
        let line = 0;
        let match = e.stack ? e.stack.match(/:(\d+):\d+/) : null;
        if (match) {
    ...
    document.getElementById("diagnostics").textContent = `${errors} errors | 0 warnings`;
}
...
function setDefaultContent() {
    ed_html.setValue(`<!-- html code goes here... -->`, -1);
    ed_css.setValue(`/* css code goes here... */`, -1);
    ed_js.setValue(`// js code goes here... `, -1);
}
...
function saveProject() {
    try {
        ...
        a.download = "academy-course.json";
        a.click(); 
